use crate::azure::auth::constants::{AUTH_SCOPES, CLIENT_ID, DEVICE_CODE_ENDPOINT, TENANT_ID, TOKEN_ENDPOINT};
...
impl TokenCredential for ManualDeviceCodeCredential {
    async fn get_token(
        ...
        info!("get token from ManualDeviceCodeCredential for scopes: {:?}", scopes);
        // Check if we already have a valid token
        {
            ...
            if let Some(token) = token_lock.as_ref() {
                if token.expires_on > OffsetDateTime::now_utc() {
                    ...
                    info!("Existing token found and valid until {:?}, returning it", token.expires_on);
        ...
        info!("Polling Azure for token using device code: {}", state.device_code);
        ...
        loop {
            ...
            if response.status().is_success() {
                ...
                let access_token = AccessToken::new(Secret::new(access_token_str.clone()), expires_on);
                
                let mut token_lock = self.access_token.write().await;
                *token_lock = Some(access_token.clone());
                
                info!("Token stored in credential state, valid until {:?}. Token secret: {}...", expires_on, &access_token_str[..10]);
                ...
                if error_code == "authorization_pending" {
                    attempts += 1;
                    if attempts % 5 == 0 {
                        info!("Still waiting for user to complete authentication (attempt {})...", attempts);
                    }
                    tokio::time::sleep(std::time::Duration::from_secs(state.interval)).await;
                } else {
                    error!("Authentication failed with status {}: {} - {}", status, error_code, error_json["error_description"].as_str().unwrap_or(""));
...
pub async fn start_device_code_login() -> Result<DeviceCodeInfo, String> {
    ...
    let client = reqwest::Client::new();
    let url = format!("{}/{}/oauth2/v2.0/devicecode", DEVICE_CODE_ENDPOINT, TENANT_ID);

    info!("Requesting device code from: {} with scope: {}", url, AUTH_SCOPES);
    let response = client
        .post(&url)
        .form(&[
            ("client_id", CLIENT_ID),
    ...
    if !response.status().is_success() {
        ...
        error!("Device code request failed with status {}: {}", status, error_text);
        return Err(format!("Device code request failed: {}", error_text));
    }

    let device_code_res: DeviceCodeResponse = response
        .json()
        .await
        .map_err(|e| {
...

    {
        ...
        *auth_lock = Some(Arc::new(credential));
        info!("ManualDeviceCodeCredential stored in global AUTH_CREDENTIAL. Is Some: {}", auth_lock.is_some());
        
        // Final verification after setting it
        if auth_lock.is_some() {
            info!("AUTH_CREDENTIAL successfully set in start_device_code_login");
        } else {
            error!("CRITICAL: AUTH_CREDENTIAL is STILL NONE after setting it in start_device_code_login!");
        }
    }
...
pub async fn complete_device_code_login() -> Result<AuthResult, String> {
    ...
    let credential = {
        ...
        info!("AUTH_CREDENTIAL state in complete_device_code_login: Is Some: {}", auth_lock.is_some());
    ...
    store_auth_result(credential, token_response.token.secret(), "Device Code Flow").await
}